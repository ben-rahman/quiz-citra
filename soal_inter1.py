...
def hitung_indikator(output, workers, hours, labour_cost):
    """Menghitung tiga indikator produktivitas dasar."""
...
def hitung_skor_indikator(value, target):
    """Menghitung skor 0–100 berdasarkan rasio capaian terhadap target."""
...
def hitung_indeks(lp, hp, wp, weights, targets):
    """Menghitung indeks produktivitas komposit (0–100)."""
    ...
    if not np.isnan(s_lp) and w_lp > 0:
        skor_list.append(s_lp)
        bobot_list.append(w_lp)
    if not np.isnan(s_hp) and w_hp > 0:
        skor_list.append(s_hp)
        bobot_list.append(w_hp)
    if not np.isnan(s_wp) and w_wp > 0:
        skor_list.append(s_wp)
        bobot_list.append(w_wp)

    if len(skor_list) == 0:
        return np.nan

    # normalisasi bobot hanya untuk indikator yang terpakai
    bobot_arr = np.array(bobot_list)
    bobot_arr = bobot_arr / bobot_arr.sum()
    skor_arr = np.array(skor_list)

    indeks = float((bobot_arr * skor_arr).sum())
    return indeks
...
if mode == "Input Manual (Single Perusahaan/Unit)":
    ...
    with col1:
        ...
        output = st.number_input(
            "Total Output (Rp, mis. Revenue / Value Added)",
            min_value=0.0,
    ...
    with col2:
        ...
        use_auto_hours = st.checkbox(
            "Hitung total jam kerja dari jam per minggu × jumlah pekerja",
            value=True,
        )
        if use_auto_hours:
            ...
            weeks_per_period = st.number_input(
                "Jumlah Minggu dalam Periode",
                min_value=1.0,
                value=13.0,
                step=1.0,
                help="Mis. 13 minggu untuk triwulan (quarter)",
            )
            total_hours = hours_per_week * weeks_per_period * workers
        else:
            total_hours = st.number_input(
                "Total Jam Kerja (seluruh pekerja, selama periode)",
                min_value=1.0,
                value=40_000.0,
                step=1000.0,
            )
    ...
    submitted = st.button("Hitung Produktivitas")
    ...
    if submitted:
        ...
        if np.isnan(indeks):
            col_d.metric("Indeks Produktivitas (0–100)", "N/A", "Set target dulu")
        else:
            col_d.metric("Indeks Produktivitas (0–100)", f"{indeks:,.2f}")
        ...
        st.markdown(
            """
            Catatan:
            - Indeks produktivitas bersifat **relatif terhadap target** yang Bro tetapkan di sidebar.  
            - Semakin tinggi capaian dibanding target, semakin tinggi skor (maksimal 100).
            """
        )
...
# ----------------------------------
    ...
    st.markdown(
        ...
        **Struktur CSV yang diharapkan:**
        - `unit` : nama perusahaan/divisi/unit  
        - `output` : total output (Rp)  
        - `workers` : jumlah tenaga kerja  
        - `hours` : total jam kerja (opsional, boleh 0)  
        - `labour_cost` : total biaya tenaga kerja (Rp)  
        """
    )
    ...
    if uploaded_file is not None:
        try:
            ...
            if missing:
                ...
                # Jika kolom hours tidak ada, buat dengan nilai NaN
                ...
                st.markdown("### 📊 Grafik Produktivitas Tenaga Kerja per Unit")
                st.bar_chart(
                    df.set_index("unit")["prod_per_worker"]
                )

                if df["productivity_index"].notna().any():
                    st.markdown("### ⭐ Indeks Produktivitas per Unit")
                    st.bar_chart(
                        df.set_index("unit")["productivity_index"]
                    )

                st.markdown(
                    """
                    Tips:
                    - Bro bisa export data ini lagi (df.to_csv) jika dijalankan di environment Python sendiri.  
                    - Versi production bisa ditambah fitur **download CSV** dan **export laporan PDF**.
                    """
                )
